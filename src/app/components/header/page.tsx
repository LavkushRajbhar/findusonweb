  ...
  return (
    <div className="bg-[#01094a] h-auto sm:h-auto md:h-[130px] border-b-4 border-red-900">
      <div className="w-full sm:w-[98%] md:w-[95%] mx-auto px-3 sm:px-2 md:px-0">
        ...
        {/* Logo and mobile/tablet menu toggle */}
        <div className="flex justify-between items-center sm:mt-4 md:mt-3">
          <div className="relative w-2xl sm:w-52 md:w-56 lg:w-64 mx-auto sm:mx-0 mt-[-20px]">
            <Image
              src="/512.png"
              alt="logo"
              width={320}
          ...
          {/* Show toggle button for both mobile and tablet */}
          {isMobileOrTablet && (
            <button
              className="text-white p-2 sm:mt-5"
              onClick={toggleMobileMenu}
              aria-label="Toggle navigation menu">
              {mobileMenuOpen ? <FiX size={22} /> : <AiOutlineMenu size={22} />}
            </button>
          )}
        </div>
        ...
        <div
          className={`w-full sm:w-[90%] md:w-[78%] mx-auto p-1 sm:p-2 mt-1 sm:mt-2 px-0 sm:px-2 md:px-4 transition-all duration-300 ease-in-out ${
            (mobileMenuOpen && isMobileOrTablet) || !isMobileOrTablet
              ? "opacity-100 max-h-screen"
              : "opacity-0 max-h-0"
          } overflow-hidden sm:overflow-visible`}>
          <nav className="text-white flex items-center justify-evenly">
            <ul
              className={`flex flex-col ${
                isDesktop ? "sm:flex-row" : ""
              } px-2 sm:px-3 gap-1 sm:gap-2 items-start justify-evenly sm:items-center w-full`}>
              {menuItems.map((item, index) => (
                <li
                  key={index}
                  className="relative group w-full sm:w-auto"
                  onMouseEnter={() =>
                    isMobileOrTablet ? null : setOpenMenu(index)
                  }
                  onMouseLeave={() =>
                    isMobileOrTablet ? null : setOpenMenu(null)
                  }>
                  <div className="flex items-center w-full justify-between">
                    <Link
                      href={item.href || "#"}
                      className={`flex items-center gap-1 hover:text-[#00aeef] font-semibold uppercase text-sm sm:text-base pl-2 sm:pl-3 py-2 sm:py-1 md:py-0 flex-grow
                        ${
                          index > 0 &&
                    ...
                    {item.subMenu && (
                      <button
                        className="block p-1 sm:p-2 text-white"
                  ...
                  {item.subMenu && (
                    <div
                      className={`${
                        isDesktop ? "sm:absolute sm:left-0 sm:top-full" : ""
                      } z-20 w-full sm:w-auto overflow-hidden transition-all duration-300 ease-in-out ${
                        (expandedSubmenu === index && isMobileOrTablet) ||
                        (!isMobileOrTablet && openMenu === index)
                      ...
                      <ul
                        ...
                        {item.subMenu.map((sub, subIndex) => (
                          <li
                            ...
                            <Link
                              href={sub.href}
                              className="block text-[13px] sm:text-[14px] leading-tight font-bold tracking-wide px-3 sm:px-4 py-1 text-blue-950 hover:bg-blue-950 hover:text-white transition-colors duration-200">
                              {sub.name}
                            </Link>
                          </li>
                        ))}
              ...
              <li
                className="relative group w-full sm:w-auto"
                onMouseEnter={() =>
                  isMobileOrTablet ? null : setOpenMenu(menuItems.length)
                }
                onMouseLeave={() =>
                  isMobileOrTablet ? null : setOpenMenu(null)
                }>
                <div className="flex items-center w-full justify-between">
                  <Link
                    href="#"
                    className="flex text-[13px] sm:text-[15px] items-center font-bold tracking-wide py-2 sm:py-1 uppercase hover:text-[#00aeef] transition-colors duration-200 flex-grow pl-2 sm:pl-0">
                ...
                <div
                  className={`${
                    isMobileOrTablet
                      ? "block w-full" // Mobile/Tablet View
                      : "absolute right-0 sm:left-auto top-full w-[250px]" // Desktop View
                  } z-50 bg-white shadow-lg rounded p-2 transition-all duration-300 ease-in-out 
                  ${
                    (!isMobileOrTablet && openMenu === menuItems.length) ||
                    (isMobileOrTablet && expandedSubmenu === menuItems.length)
                  ...
                  <div className="bg-white shadow-lg rounded w-full py-2">
                    ...
                    />
                    <h2 className="text-[16px] sm:text-[19px] font-semibold text-gray-700 mt-2 text-center">
                      Find Us On Web Admin
                    </h2>
                    <p className="text-[13px] sm:text-[15px] text-gray-500 text-center">
                      (support@findusonweb.com)
                    </p>
                    <p className="text-[12px] sm:text-[13px] font-semibold text-green-600 mt-1 text-center">
                      Balance Shopping Credit - ₹54.00
                    </p>

                    <ul className="mt-3 sm:mt-4 text-blue-900 text-[13px] sm:text-[14px] font-bold">
              ...
              {/* Hamburger menu for desktop only */}
              {isDesktop && (
      ...
      {/* Side modal for desktop menu only */}
      {isDesktop && (
        <div
